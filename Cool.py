...
banner = ("""
 ...
 _____ _____ ____     _____ _           _           Tele: @O0Dev 
|     |     |    \   |     | |_ ___ ___| |_ ___ ___ 
|   --|  |  |  |  |  |   --|   | -_|  _| '_| -_|  _|
...
async def Osama_amy(email,pasw,token,myID):
    ...
    try:
        ...
        a +=1
        os.system('cls' if os.name == 'nt' else 'clear')
        print(f'{banner}[-----------(Coded By : Osama A.M.Y)-----------]\n\n[-] Done : {a}\t\t\t[-] Bad : {b}')
        f = open('COD@O0Dev.txt','a').write(f'{email}:{pasw}\n')
        try:
...
try:
    List = open('accounts.txt','r').read().splitlines()
except:
    print(banner)
