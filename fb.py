...
def banner():
            ...
            print (f"""{R}
  ____ _                           
 / ___| |__   __ _ _ __ ___  _ __  Charon - FB Hacker
| |   | '_ \ / _` | '__/ _ \| '_ \ CH - @Charon_Devloper
| |___| | | | (_| | | | (_) | | | |Tele - @AA0AAA
 \____|_| |_|\__,_|_|  \___/|_| |_|Insta - @LUFFY_HB""")
...
class start:
	def __init__(self):
	           ...
	           if '2' in mode:
	           	self.mode = "telegram"
	           	self.token = input(f"	{G} ~ {W} Token: ")
	           	self.id = input(f"	{G} ~ {W} ID: ")
	           else:
	           	self.mode = "file"
	           stt = f"	{G} ~ {W} Wait "
	           for line in range(4):
	...
	def st(self):
		...
		print (f"	{G} ~ {W} Telegram : https://t.me/Charon_Devloper \n")
		time.sleep(2)
		for line in email:
